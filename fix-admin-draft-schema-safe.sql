...
-- (without foreign key constraints initially)
...
-- 5. Clean up any invalid player_id references before adding constraints
UPDATE draft_picks 
SET player_id = NULL 
...
-- 6. Now add foreign key constraints safely
ALTER TABLE draft_picks 
...
-- 8. Update RLS policies to allow proper access
...
-- 9. Add admin policies for draft allocation
...
-- 10. Ensure chelsea_players table has proper structure
ALTER TABLE chelsea_players 
ADD COLUMN IF NOT EXISTS is_available BOOLEAN DEFAULT true;

-- 11. Update chelsea_players to be available by default
UPDATE chelsea_players 
SET is_available = true 
WHERE is_available IS NULL;

-- 12. Insert some sample users for testing if none exist
...
SELECT 'Available Chelsea players:' as info, COUNT(*) as count FROM chelsea_players WHERE is_available = true;
