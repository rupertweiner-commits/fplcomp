...
DROP TABLE IF EXISTS public.chip_history CASCADE;
DROP TABLE IF EXISTS public.user_activity CASCADE;
DROP TABLE IF EXISTS public.draft_queue CASCADE;
...
DROP TABLE IF EXISTS public.fpl_cache CASCADE;
...
CREATE TABLE public.chip_history (
    id UUID DEFAULT uuid_generate_v4() PRIMARY KEY,
    user_id UUID REFERENCES public.user_profiles(id) ON DELETE CASCADE,
...
CREATE INDEX idx_chelsea_players_drafted_by ON public.chelsea_players(drafted_by);
CREATE INDEX idx_draft_queue_user_id ON public.draft_queue(user_id);
CREATE INDEX idx_user_activity_user_id ON public.user_activity(user_id);
CREATE INDEX idx_fpl_cache_expires_at ON public.fpl_cache(expires_at);
...
-- Verify the setup
SELECT 'Database reset and tables created successfully!' as status;
