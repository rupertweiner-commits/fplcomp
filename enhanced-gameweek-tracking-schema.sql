...
CREATE TABLE IF NOT EXISTS public.user_team_snapshots (
    ...
    team_composition JSONB NOT NULL, -- Array of player objects with positions, captain status, etc.
    
    -- Formation
    formation TEXT, -- e.g., "3-4-3", "4-3-3", etc.
    
    -- Transfers made this gameweek
    transfers_made INTEGER DEFAULT 0,
    transfer_cost INTEGER DEFAULT 0, -- Points cost for transfers
    
    -- Chips used
    chip_used TEXT, -- 'wildcard', 'free_hit', 'bench_boost', 'triple_captain'
...
CREATE TABLE IF NOT EXISTS public.user_transfer_history (
    ...
    -- Transfer costs
    transfer_cost INTEGER DEFAULT 0, -- Points cost
...
COMMENT ON TABLE public.player_gameweek_performance IS 'Detailed player performance data for each gameweek';
COMMENT ON TABLE public.user_team_snapshots IS 'Snapshot of user team composition for each gameweek';
COMMENT ON TABLE public.user_gameweek_scores IS 'User scores and performance metrics per gameweek';
COMMENT ON TABLE public.user_transfer_history IS 'Complete history of all user transfers';
COMMENT ON TABLE public.user_chip_history IS 'History of chip usage by users';
COMMENT ON TABLE public.user_total_points IS 'Aggregated user statistics and rankings';
COMMENT ON TABLE public.gameweek_rankings IS 'Weekly rankings for all users';
COMMENT ON TABLE public.season_overview IS 'Season-wide configuration and statistics';
