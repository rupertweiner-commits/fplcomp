...
function ManagerAnalysis() {
  ...
  const fetchManagerData = async() => {
    try {
      ...
      const gwResponse = await fetch('/api/players?action=current-gameweek');
  ...
  if (error) {
    return (
      <div className="bg-red-50 border border-red-200 rounded-lg p-6 text-center">
        ...
        <button className="fpl-button-primary" onClick={fetchManagerData}>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-4 sm:space-y-0">
          <div className="flex items-center space-x-4">
            ...
            <div>
              ...
              <div className="flex items-center space-x-4 mt-1 text-sm text-gray-500">
                ...
                <span>
                  Started:
                  {managerData.started_event ? `GW${managerData.started_event}` : 'GW1'}
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="text-right">
            ...
            <div className="text-xs text-gray-500">
              Overall Rank:
              {' '}
              {managerData.summary_overall_rank?.toLocaleString() || 'N/A'}
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="text-right">
          <div className="flex items-center space-x-1">
            <span className="text-sm text-gray-900">
              Rank:
              {' '}
              {gameweek.overall_rank?.toLocaleString()}
            </span>
            {getRankChange()}
          </div>
          <div className="text-xs text-gray-500">
